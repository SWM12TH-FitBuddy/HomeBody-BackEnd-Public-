...
public class ExerciseStatDto {
    ...
    public ExerciseHistory toEntity() {
        return ExerciseHistory.builder().userId(userId).type(type).dayOfRoutine(dayOfRoutine).records(records).level(level).startTime(startTime.toString()).endTime(endTime.toString()).build();
    }

    @Builder
    public ExerciseStatDto(Long idx, String userId, String type, Integer dayOfRoutine, String records, String level, LocalDate startTime, LocalDate endTime) {
        this.idx = idx;
        this.userId = userId;
        this.type = type;
